...
class controllers {
  async signup(req, res) {
    try {
      ...
      const hashConfirmPassword = bcrypt.hashSync(confirmPassword, HASH_COMPLEXITY_NUMBER);
      const user = new User({
        userName,
        email,
  ...
  async signin(req, res) {
    try {
      ...
      if (!user) {
        return res
          .status(400)
          .json({ message: req.t('error_noSuchUser') });
      }
  ...
  async forgot(req, res) {
    try {
      ...
      if (!errors.isEmpty()) {
        return res
          .status(400)
          .json({ message: 'Errors during email request', errors: errors.array() });
      }
      ...
      return res
        .status(200)
        .json({ message: 'Mail was sent successfully' });
    } catch (err) {
      console.log(err);
    }
  }
  ...
  async reset(req, res) {
    try {
      ...
      if (!errors.isEmpty()) {
        return res
          .status(400)
          .json({ message: 'Errors during password reset', errors: errors.array() });
      }
      ...
      if (newPassword === confirmPassword) {
        const hashNewPassword = bcrypt.hashSync(newPassword, HASH_COMPLEXITY_NUMBER);
  ...
  async updateUserData(req, res) {
    try {
      ...
      if (!errors.isEmpty()) {
        return res
          .status(400)
          .json({ message: 'Errors during user data update', errors: errors.array() });
      }
      ...
      if (password !== null && !bcrypt.compareSync(password, user.password)) {
        const hashNewPassword = bcrypt.hashSync(password, HASH_COMPLEXITY_NUMBER);
        await User.findOneAndUpdate({ email }, { password: hashNewPassword }, {
          new: true,
        });
        return res
  ...
  async planCreation(req, res) {
    try {
      ...
      const plan = await Plan.findOne({ firstQuestion, secondQuestion, thirdQuestion });
      if (!plan) {
